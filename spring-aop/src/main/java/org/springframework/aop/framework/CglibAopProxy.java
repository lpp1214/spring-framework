...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			//设置拦截器，cglib中对于方法的拦截是通过将自定义拦截器（实现MethodInterceptor接口）加入Callback中，
			//并在调用代理时直接激活拦截器中的intercept方法来实现
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		@Override
		@Nullable
		//本方法与JDK代理中的invoke方法大同小异，都是首先构造链，然后封装此链进行串联调用，
		//稍有些区别是JDK中直接构造ReflectiveMethodInvocation，而在cglib中使用CglibMethodInvocation，
